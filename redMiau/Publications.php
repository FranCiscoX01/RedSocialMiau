...
include('conexion.php');
  ...
  if (mysqli_num_rows($public) > 0) {
    while ($rowP = mysqli_fetch_assoc($public)) {
      $likes = mysqli_query($conexion, "SELECT * FROM likes WHERE Username = '".$_SESSION['user']."' AND idPublic = ".$rowP['idPublic']."");
      ...
      if ($rowP['Tex_t'] != NULL && $rowP['Image'] == NULL && $rowP['Video'] == NULL && $rowP['Audio'] == NULL) { //SOLO TEXTO
        if (mysqli_num_rows($likes) == 0){
          ...
          <div class='card'>
            ...
            <center><a href='likes.php?id=".$rowP['idPublic']."'><button type='button' class='btn btn-outline-danger'>
            Me gusta
